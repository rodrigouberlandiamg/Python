...
# utilizando a linguagem de programação C++. Para computar cada voto, a telefonista
# digitará um número, entre 1 e 23, correspondente ao número da camisa do jogador.
# Um número de jogador igual zero, indica que a votação foi encerrada. Se um número
# inválido for digitado, o programa deve ignorá-lo, mostrando uma breve mensagem de aviso,
...
def calcula_perc(votos, total_votos):
    return (votos * 100) / total_votos

def log(texto):
    with open('c:/temp/resultado_prog.txt','a+') as file:
        print(texto)
        file.write(f'{texto}\n')
...
print('')
log('Resultado da votação:\n')
log(f'Foram computados {len(jogadores)} votos.\n')
log('JOGADOR      VOTOS       %')
...
for x in resultado:
    #print(f'{x}')
    #percentual = (resultado[x] * 100) / soma
    log(f'{x}            {resultado[x]}            {calcula_perc(resultado[x],soma):.2f}%')
