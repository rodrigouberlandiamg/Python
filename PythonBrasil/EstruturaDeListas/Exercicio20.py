...
# você para desenvolver a aplicação que servirá como uma projeção de quanto será gasto com
# o pagamento deste abono.
#
...
#     de salários. Um valor de salário igual a 0 (zero) encerra a digitação. Após a entrada de
#     todos os dados o programa deverá calcular o valor do abono concedido a cada colaborador,
...
while True:
    salario = float(input(f'Entre com o salario: R$'))
    if salario == 0:
        break
    else:
...
count_min = 0
soma_abonos = 0
maior_abono_pago = 100
print('SALARIO      ABONO')
for salario in salarios:
    abonos = 0
    print(f'RS{salario}',end='      ')
    if (salario*0.2) < 100:
        count_min += 1
        abonos = 100
        soma_abonos += 100
        ...
        abonos = salario*0.2
        soma_abonos += salario*0.2
        if abonos > maior_abono_pago:
            maior_abono_pago = abonos
    print(f'RS{abonos}')

print(f'Foram processados {len(salarios)} colaboradores')
print(f'Total gasto com abonos: R$ {soma_abonos}')
print(f'Valor mínimo pago a {count_min} colaboradores')
print(f'Maior valor de abono pago: R$ {maior_abono_pago}')